...
在上个 [Rclone 与 b2_bucket 结合的文章](/docs/玩客记录/使用rclone+b2_bucket进行文件备份.md) 中介绍了 B 储存的几大优势，所以我们这次仍然选择 B2 作为储存

并且我们发现，Cloudflare 与 B2 之间的流量是完全免费的，且 Cloudflare 的 Workers 免费用户每天能有 100,000 次的请求，这足以支撑一个小型博客 + 知识库的图片需求

这篇文章就主要讲述了如何将两者结合，构建一个个人图床。

## 具体操作

### 在 B2 添加储存桶，并且创建对应应用密钥

在正常添加完成 B2 储存桶以后，你需要记住对应桶的 Endpoint 信息，这在我们之后的过程中会
使用

同时，你需要对桶进行如下设置：

- 在 Bucket Settings 中的 Bucket Info 下填入如下信息用于控制桶的缓存时间：

```
{"cache-control":"public, max-age=86400"}
```

这里的 `max-age=86400` 是控制缓存为 1 天，单位为秒

- 在 Lifecycle Settings 下将选项改为 `Keep only the last version of the file`

在完成添加应用密钥之后，你需要记下对应的 KeyID 和 ApplicationsID，这在我们之后的过程会中使用

### 在 Cloudflare 创建 Cloudflare Worker

进入 Cloudflare 后，选择菜单左侧的 `Workers和Pages`，点击创建，选择 Workers，点击创建 Worker
...
然后点击部署，你的 Worker 就创建完毕了

### 配置 Cloudflare Worker

^^先决条件：你需要有一个托管在 Cloudflare 的个人域名^^

点击变量与机密，在该选项下添加如下内容：

```
ALLOW_LIST_BUCKET = false
B2_APPLICATION_KEY =  你的 ApplicationsID
B2_APPLICATION_KEY_ID = 你的 keyID
B2_ENDPOINT = 你的桶的 Endpoint 值
BUCKET_NAME = 你的桶名
```

完成填写后，请点击保存并部署

之后点击域和路由，在该选项下点击添加，选择自定义域，输入你的个人域名，随后点击添加域

至此，你的 Cloudflare Worker 就已经配置完成了